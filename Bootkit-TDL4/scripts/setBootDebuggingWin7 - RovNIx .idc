...
static main() {
  ...
  if ( !SegCreate(0x0000,0xA000,0x0000,0,saRelByte,scPub) ) {
    Warning("Can't create Real Mode Interrupt Vector Table");
    return;
  } 
  SegRename(0x0000,"_0x0000");
  ...
  if ( !SegCreate(0x00020000,0x00024CB0,0x2000,0,saRelByte,scPub) ) {
    Warning("Can't create bootmgr16");
    return;
  } 
  SegRename(0x00020000,"bootmgr16");

  if ( !SegCreate(0x00400000,0x00477000,0x0,1,saRelByte,scPub) ) {
    Warning("Can't create bootmgr32");
    return;
  } 
  SegRename(0x00400000,"bootmgr32");
  
  if ( !SegCreate(0x0009F000,0x9F800,0x9F00,0,saRelByte,scPub) ) {
    Warning("Can't create Malware Segment");
    return;
  } 
  SegRename(0x0009F000,"rovnix16");
  
  //if ( !SegCreate(0x000F0000,0x000FFFFF,0xFFFF,0,saRelByte,scPub) ) {
  //  Warning("Can't create System Bios");
  //  return;
  //} 
  //SegRename(0x000F0000,"SysBios"); 
}
